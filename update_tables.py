...
for log_path in log_files:
    # Example: cifar100_20c_noniid_lower_FeDoRA_seed42.log
    ...
    # Extract test accuracy (we want "Global Model Test accuracy: X" or "MEAN_LOCAL_ACC = X")
    ...
    # Check if we should use Latest Acc for RUNNING
    if not is_finished and test_accs:
        # Just use the latest raw accuracy for running indicator
        val = test_accs[-1]
    ...
    if val is not None:
        ...
        col_key = f"{dataset.upper()} ({'IID' if partition=='iid' else 'Non-IID'})"
        results[clients][method][col_key] = (val, is_finished)

# Map method names from log to table
method_map = {
    "FeDoRA": "Ours (FeDoRA)",
    ...
    if in_table and line.strip().startswith("|"):
        ...
        if log_method and current_clients in results and log_method in results[current_clients]:
            ...
            for i, col_name in enumerate(columns):
                ...
                # Dataset col
                col_key = col_name # e.g. "CIFAR-100 (IID)"
