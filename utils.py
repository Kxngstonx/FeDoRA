...
def get_global_dataset(args):
    ...
    if args.dataset == 'mmlu':
        ...
        train_ds = MMLUDataset(train=True, model_name=model_name, max_length=128)
        val_ds = None
        test_ds = MMLUDataset(train=False, model_name=model_name, max_length=128)
        return train_ds, val_ds, test_ds
    
    if args.dataset == 'anli':
        from datasets.anli import ANLICombinedTrainDataset, ANLIRoundDataset
        train_ds = ANLICombinedTrainDataset(model_name=model_name, max_length=128)
        val_ds = None
        # Default test: use R1 test set for global_test_dataloader
        test_ds = ANLIRoundDataset('R1', split='test', model_name=model_name, max_length=128)
        return train_ds, val_ds, test_ds
    ...
    if args.dataset.startswith('superglue_'):
        from datasets.superglue import SuperGLUEDataset
        task = args.dataset[len('superglue_'):]
        train_ds = SuperGLUEDataset(task=task, train=True,  model_name=model_name, max_length=128)
        val_ds   = None
        test_ds  = SuperGLUEDataset(task=task, train=False, model_name=model_name, max_length=128)
        return train_ds, val_ds, test_ds
    ...
    if args.dataset == 'fmnist':
        ...
        train_ds = SVHN_truncated(
            args.datadir, train=True, transform=transform_train, download=True)
        val_ds = None
        test_ds = SVHN_truncated(
            args.datadir, train=False, transform=transform_test, download=True)
...
def init_nets(dataset, num_nets, args, device='cpu', base=False, use_projection_head=False):
    ...
    for net_i in range(num_nets):
        if args.model == 'resnet50':
            ...
            net = ViTWrapper(num_classes=num_classes, model_name='vit_small_patch16_224')
        elif args.model == 'vit-base':
            net = ViTWrapper(num_classes=num_classes, model_name='vit_base_patch16_224')
        else:
            supported = ['resnet18', 'resnet34', 'resnet50',
                         'resnet101', 'resnet152', 'mobilenet', 'qwen', 'vit', 'vit-base']
            raise ValueError(
...
def get_global_class_center(global_class_center_old, n_party, nets, args, device, train_dataloader):
    DATA_nclass = {'mnist': 10, 'cifar10': 10, 'svhn': 10,
                   ...
                   'superglue_boolq': 2, 'superglue_multirc': 2, 'superglue_wic': 2, 'superglue_record': 2,
                   'anli': 3}
