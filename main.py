...
import time
import os
# Force HuggingFace to use NVMe SSD cache to save space on root partition
os.environ['HF_HOME'] = '/mnt/data1/hbkim/hf_cache'
os.environ['HF_DATASETS_CACHE'] = '/mnt/data1/hbkim/hf_cache/datasets'
...
def get_args():
    ...
    parser.add_argument('--peft', type=str, choices=['none', 'lora', 'dora', 'ravan'], default='none')
    ...
    parser.add_argument('--ravan_init', type=str, choices=['gram_schmidt', 'random_normal'], default='gram_schmidt', help='RAVAN B/A initialization method')
    parser.add_argument('--target_modules', type=str, nargs='*', default=None, help='Target module names for PEFT injection (e.g. query value)')
...
def main():
    ...
    if args.peft != 'none':
        for param in base_global_model.parameters():
            param.requires_grad = False
    ...
    for round in range(args.round):
        ...
        # FedEx-LoRA 집계: W0 영구 업데이트 및 A_global, B_global 덮어쓰기
        if getattr(args, 'fedex_lora', False) and args.peft == 'lora':
        ...
        elif getattr(args, 'flex_lora', False) and args.peft in ['dora', 'lora']:
        ...
        # RAVAN 집계: s_i * H_i 평균 후 s_i reset to 1
        if args.peft == 'ravan':
            from peft_utils import ravan_aggregate
            updated_ravan_params = ravan_aggregate(
                updated_client_weights, fed_avg_freqs, global_model)
            global_model.load_state_dict(updated_ravan_params, strict=False)
            global_w.update(updated_ravan_params)
        ...
        # ANLI per-round test evaluation (R1, R2, R3)
        if args.dataset == 'anli':
            if not hasattr(args, '_anli_round_test_loaders'):
                import torch.utils.data as _data
                from datasets.anli import ANLIRoundDataset
                model_name = get_hf_model_name(getattr(args, 'model', 'roberta-base'))
                args._anli_round_test_loaders = {}
                for rnd in ['R1', 'R2', 'R3']:
                    rnd_ds = ANLIRoundDataset(rnd, split='test', model_name=model_name, max_length=128)
                    args._anli_round_test_loaders[rnd] = _data.DataLoader(
                        dataset=rnd_ds, batch_size=args.test_batch_size, shuffle=False,
                        pin_memory=True, num_workers=args.num_workers)
            
            anli_accs = []
            for rnd, rnd_loader in args._anli_round_test_loaders.items():
                rnd_acc = compute_accuracy(global_model, rnd_loader, device=device)
                pkl_dict.setdefault(f'acc_{rnd}', []).append(rnd_acc)
                logger.info(f'>> Global Model Test accuracy ({rnd}): {rnd_acc:.4f}')
                anli_accs.append(rnd_acc)
            
            if anli_accs:
                avg_acc = sum(anli_accs) / len(anli_accs)
                pkl_dict.setdefault('acc_ANLI_avg', []).append(avg_acc)
                logger.info(f'>> Global Model Test accuracy (ANLI Average): {avg_acc:.4f}')
